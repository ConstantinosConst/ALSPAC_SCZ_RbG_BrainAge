...
#   variable <- females_hc
#   
model.fits.brainAge.SCZ_PRS=function(model, brain_age_output){
  variable=model
  ...
  variable$SCZ_PRS[variable$SCZ_PRS==0] <- "Low-PRS"
  variable$SCZ_PRS[variable$SCZ_PRS==1] <- "High-PRS"
  
  for (i in c("Low-PRS","High-PRS")){
    
    # Calculate model metrics
    
      variable.SCZ_PRS=variable[variable$SCZ_PRS==i,]
      cat("For model >",brain_age_output,", ",i,"< your MAE is: ",mae(variable.SCZ_PRS$Age, variable.SCZ_PRS$predAge), "\n")
      cat("For model >",brain_age_output,", ",i,"< your MSE is: ",mse(variable.SCZ_PRS$Age, variable.SCZ_PRS$predAge), "\n")
      cat("For model >",brain_age_output,", ",i,"< your age~predAge cor is: ",cor(variable.SCZ_PRS$Age, variable.SCZ_PRS$predAge), "\n")
      cat("For model >",brain_age_output,", ",i,"< your R2 is: ",caret::R2(variable.SCZ_PRS$Age, variable.SCZ_PRS$predAge), "\n")
      brain_PAD <- (variable.SCZ_PRS$predAge-variable.SCZ_PRS$Age)
    ...
    ##Make a plot of MAE distribution per SCZ_PRS status
      variable$SCZ_PRS=factor(variable$SCZ_PRS)
      p00 <- ggplot(variable, aes(y=AE, x=SCZ_PRS)) + geom_boxplot(alpha=0.5)+ geom_smooth() + labs(x = "", y = "") + 
    ...
    ##Make a plot of MAE distribution per age bin and SCZ_PRS statues
    variable$agegroups=factor(variable$agegroups)
    p02 <- ggplot(variable, aes(y=AE, x=agegroups, fill=SCZ_PRS)) + geom_boxplot(alpha=0.5)+ geom_smooth() + labs(x = "", y = "") +
    ...
    ## Make a plot of devAge distribution per SCZ_PRS status
       variable$SCZ_PRS=factor(variable$SCZ_PRS)
       p03 <- ggplot(variable, aes(y=devAge, x=SCZ_PRS)) + geom_boxplot(alpha=0.5)+ geom_smooth() + labs(x = "", y = "") +
         geom_jitter(color="black", size=0.4, alpha=0.9)

       p03 <- p03 + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                                       panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))

    ##Make a plot of devAge distribution per age bin and SCZ_PRS
    variable$agegroups=factor(variable$agegroups)
    p04 <- ggplot(variable, aes(y=devAge, x=agegroups, fill=SCZ_PRS)) + geom_boxplot(alpha=0.5)+ geom_smooth() + labs(x = "", y = "") +
      geom_jitter(color="black", size=0.2, alpha=0.9)
    ...
    pdf(paste0(deparse(substitute(model)),"_SCZ_PRS_BA_descriptives.pdf"))
    ...
    # How to evaluate models: observed vs. predicted or predicted vs. observed?. Ecological Modelling, 216(3-4), 316-322.
       ...
       variable$SCZ_PRS <- factor(variable$SCZ_PRS)
       p1 <- ggplot(variable, aes(x=Age, y=predAge, color=SCZ_PRS, group=SCZ_PRS))+ 
         ...
         scale_colour_hue(l=50) + geom_smooth(aes(fill=SCZ_PRS), method=lm, size=0.5, se=TRUE, fullrange=TRUE)+
         # geom_smooth(method=lm, colour="black", size=0.5, se=TRUE, fullrange=TRUE) +
